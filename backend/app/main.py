...
def _year_for_month(month: int, today: date) -> int:
    # If month is already completed, use current year; otherwise previous year.
...
@app.get("/api/metrics", response_model=list[FinancialMovement])
def get_metrics(
...
@app.get("/api/metrics/b2b", response_model=list[FinancialMovement])
def get_b2b_metrics(
    start_date: date | None = Query(default=None),
    end_date: date | None = Query(default=None),
) -> list[FinancialMovement]:
...
@app.get("/api/metrics/b2c", response_model=list[FinancialMovement])
def get_b2c_metrics(
    start_date: date | None = Query(default=None),
    end_date: date | None = Query(default=None),
) -> list[FinancialMovement]:
